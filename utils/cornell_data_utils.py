...
def get_conversations():
    ...
    '''
    conversations = []
    with open('raw_cornell_data/movie_conversations.txt', 'r', encoding='utf-8') as f:
...
def get_movie_lines():
    ...
    '''
    sentences = {}
    with open('raw_cornell_data/movie_lines.txt', 'r', encoding='utf-8') as f:
        for line in f.readlines():
    ...
    for i in range(len(convs)):
        ...
        if len(conversation) % 2 == 0:
            for line in range(len(conversation)):
                if line % 2 == 0:
                    with open('movie_questions.txt', 'a', encoding='utf-8') as f:
                        f.write(lines[conversation[line]] + "\n")
                else:
                    with open('movie_answers.txt', 'a', encoding='utf-8') as f:
                        f.write(lines[conversation[line]] + "\n")
...
def questions_vs_answers_v2(convs, lines):
    ...
    for i in range(len(convs)):
        ...
        for line in range(len(conversation) - 1):
            with open('movie_questions_2.txt', 'a', encoding='utf-8') as f:
                f.write(lines[conversation[line]] + "\n")
            with open('movie_answers_2.txt', 'a', encoding='utf-8') as f:
...
def clean_data():
    ...
    with open('movie_questions_2.txt', 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            cleaned_questions.append(cornell_tokenizer(line))

    with open('movie_answers_2.txt', 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
...
def create_vocab(questions, answers):
    ...
    for i in range(len(questions)):
        for j in range(len(questions[i])):
            vocab.append(questions[i][j])

        for j in range(len(answers[i])):
            vocab.append(answers[i][j])
        ...
        for i in range(len(questions)):
            if len(questions[i]) <= encoder_max and len(answers[i]) <= decoder_max:
                if i not in already_added:
                    data_for_bucket.append((pad_data(questions[i], word_to_id, encoder_max),
                                            pad_data(answers[i], word_to_id, decoder_max, True)))
...
def bucket_one_question(question, word_to_id, bucket_id):
    encoder_max = config.BUCKETS[bucket_id][0]
    return pad_data(question, word_to_id, encoder_max)
